...
public abstract class AutonomousPositions {
    ...
    public static SwerveTrajectory generate(TrajectoryConfig config, Rotation2d rotation, double turnDelay,
            Pose2d... poses) {
        Trajectory traj = new Trajectory();
        Pose2d startPose = poses[0];
        Pose2d endPose = poses[poses.length - 1];
        ...
        return new SwerveTrajectory(traj).setTrajectoryConfiguration(config).addRotation(rotation, 2 * Math.PI,
                turnDelay);
    }
}