...
public class Launcher {
    ...
    public void update(){
        ...
        switch(state){
            case UP:
                if(launchMotor1.getSelectedSensorPosition()<Constants.TICKS_UP_POSITION){
                    launchMotor1.set(ControlMode.PercentOutput, 0.1);
                }
                else{
                    launchMotor1.set(ControlMode.PercentOutput, 0.05);
                }
                break;
            ...
            case LAUNCH: //Activated from a button combo in Robot.java.
                ...
                if (localPos >= Constants.LAUNCH_MAX_POSITION) { //If it's time to stop
                    ...
                    launchMotor1.set(ControlMode.PercentOutput, 0.7);
                }
                break;

            case IDLE_WAIT: // Does nothing for 0.5 seconds, then switches the state to lowering the arm
