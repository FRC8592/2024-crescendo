            ...
            case SPEAKER:
                switch (subState) {
                    ...
                    case SCORE: // Note that this runs READY and PREP's code too because there's no `break;` after this code
                        ...
                        if (timer.get() > SHOOTER.SHOOT_SCORE_TIME) { //If we have spent enough time shooting
                            timer.stop();
                            timer.reset();
                    ...
                    case PREP:
                        RangeTable.RangeEntry target = RangeTable.get(distanceToSpeaker);//TODO: Remember, distanceToSpeaker is just an index in the range table for now.
