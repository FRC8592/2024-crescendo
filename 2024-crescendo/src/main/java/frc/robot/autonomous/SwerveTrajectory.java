...
/**
 * Custom Trajectory generator class based on the WPILib {@code Trajectory}
 * class
 */
public class SwerveTrajectory {
    ...
    public SwerveTrajectory(Trajectory trajectory) {
        ...
        mTurnPID = new ProfiledPIDController(0.5, 0, 0, new Constraints(4 * Math.PI, 2 * Math.PI)); // Probably should
                                                                                                    // increase the P
                                                                                                    // value or maybe
                                                                                                    // even change
                                                                                                    // constraints to
        ...
        // rotation =
        // trajectory.sample(trajectory.getTotalTimeSeconds()).poseMeters.getRotation();
     ...
     * @return the same {@code SwerveTrajectory} object back but with the added
     *         {@code Rotation2d} for easy usage
     */
     ...
     * @return a {@code ChassisSpeeds} object for x, y, and omega speeds in imperial
     *         units (m/s and rad/s)
     */
    public ChassisSpeeds sample(double pSeconds, Pose2d robotPose) {
        ...
        if (Robot.isReal()) {
            ...
            // desired = ChassisSpeeds.fromFieldRelativeSpeeds(desired,
            // robotPose.getRotation());
            // SmartDashboard.putNumber("Current Heading",
            // robotPose.getRotation().getDegrees());
        }

        // SmartDashboard.putNumber("Error X", getEndingPose().getX() -
        // robotPose.getX());
        // SmartDashboard.putNumber("Error Y", getEndingPose().getY() -
        // robotPose.getY());
        // SmartDashboard.putNumber("Error Theta", rotation.getDegrees() -
        // robotPose.getRotation().getDegrees());
    ...
    public boolean isFinished(double time) {
        if (Robot.isReal()) {
            ...
            // ||
            time >= mTrajectory.getTotalTimeSeconds()
                    || (Math.abs(getEndingPose().getX() - poseRobot.getX()) <= acceptanceRange && vision);
        } else {
    ...
    /**
     * @return underlying WPILib {@code Trajectory} object that the current
     *         {@code SwerveTrajectory} is based on
     */
